...
export default function UsuarioPage() {
  ...
  if (!usuario || usuario.rol?.toLowerCase() !== 'usuario') {
    return (
      <div className="min-h-[calc(100vh-3.5rem)] w-full bg-stone-50 dark:bg-zinc-950 px-4 py-10 flex items-center justify-center">
        <div className="max-w-md w-full rounded-2xl border border-red-200 bg-red-50 p-6 text-center text-red-800 dark:border-red-950 dark:bg-red-950/20 dark:text-red-300">
          ...
          <p className="mt-2 text-sm">Debes iniciar sesión con una cuenta de tipo Usuario para acceder a este panel.</p>
          <Link href="/login?rol=usuario" className="mt-4 inline-flex rounded-lg bg-amber-600 px-4 py-2 text-sm font-medium text-white hover:bg-amber-700">
            Iniciar Sesión
          </Link>
        </div>
      </div>
