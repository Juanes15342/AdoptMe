...
async function testMascotasList() {
  const store = getStore();
...
async function testMascotasCreate(body) {
  const store = getStore();
...
async function testMascotasGetById(id) {
  const store = getStore();
...
async function testMascotasUpdate(id, body) {
  const store = getStore();
  const mascota = store.mascotas.find((m) => m.id === id);
  if (!mascota) return { error: "Mascota no encontrada", status: 404 };
...
async function testMascotasDelete(id) {
  const store = getStore();
...
async function testAdopcionesList() {
  const store = getStore();
...
async function testAdopcionesCreate(body) {
  const store = getStore();
...
async function testAdopcionesGetById(id) {
  const store = getStore();
...
async function testAdopcionesUpdate(id, body) {
  const store = getStore();
  const adopcion = store.adopciones.find((a) => a.id === id);
  if (!adopcion) return { error: "Adopción no encontrada", status: 404 };
...
async function testAdopcionesDelete(id) {
  const store = getStore();
...
async function testUserCreate(body) {
  const store = getStore();
...
async function testUserGetById(id) {
  const store = getStore();
...
async function testUserUpdate(id, body) {
  const store = getStore();
  const usuario = store.usuarios.find((u) => u.id === id);
  if (!usuario) return { error: "Usuario no encontrado", status: 404 };
...
async function testUserDelete(id) {
  const store = getStore();
...
async function testAuthLogin(body) {
  await initAuthUser();
...
function testSetAuthPasswordHash(hash) {
  const store = getStore();
  store.authPasswordHash = hash;
}
